...
After an artifact is published or attached, verify the actual delivered names before presenting it as final:

```text
python -m tools.resume_release verify-name-integrity \
  --contract RESUME_RELEASE_CONTRACT.json \
  --receipt STAGING/artifact-name-integrity-receipt.json \
  --report-out STAGING/artifact-name-integrity-report.json
```

The receipt uses `schemas/resume-release/artifact-name-integrity-receipt.schema.json` and records the canonical filename plus one observed, not-applicable, or uninspectable result for every naming surface. `uninspectable` produces `UNKNOWN`; any encoded-space leak or canonical mismatch produces `FAIL`. Both block final presentation.
