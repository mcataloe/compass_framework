...
Current COMPASS Version: vNext 2026-09.2
...
- `rules/20-professional-effectiveness-evidence.md` defines candidate-neutral handling for non-technical hiring signals including critical thinking, problem framing, systems thinking, judgment under ambiguity, communication, collaboration, influence, ownership, adaptability, stakeholder management, mentoring, and related professional-effectiveness capabilities.
- Professional-effectiveness capability labels are derived analytical metadata. The verified action, decision, constraint, stakeholder context, and consequence remain the factual authority, and target job descriptions cannot create capability evidence.
- COMPASS Analysis maps explicit and implicit professional-effectiveness signals inside the existing 13-section contract. Resume, cover-letter, recruiter-message, application-answer, and interview-preparation workflows should demonstrate the behavior through source-backed evidence rather than generic soft-skill adjective lists.
- Existing Source-of-Truth records remain compatible without mass capability-tag backfill. Explicit annotations may be added just in time when current Source-of-Truth policy authorizes them and the underlying evidence is already verified.
- `rules/21-interview-answer-retrieval.md` defines candidate-neutral recall-first interview preparation. Proposed answers should lead with a one-sentence Core, a short semantic Memory hook, no more than three primary beats, and when useful one coherent source-backed Story hook before any optional expanded demonstration wording.
- Interview-preparation prose is illustrative rather than a memorization target. Expanded sample wording should be labeled `Do not memorize verbatim`, and a pivot from a general approach question to `give me an example` should switch directly into one verified story instead of repeating the abstract answer.
- Interview answer compression changes presentation only. It must preserve TruthGuard, claim depth, do-not-claim boundaries, collaborators, implementation stage, measured-versus-observational outcomes, declared interview lens, and user-specific interview policy.
...
- `python -m tools.resume_release` implements version `1.2.0` validation, atomic local release, and post-publication artifact-name verification using only the Python standard library, with blocking `FAIL` or `UNKNOWN` behavior when required rendering or delivery-name verification is unavailable or invalid.
- Experience-duration validation supports both overlap-safe union intervals and exact completed years from one source-backed career-start anchor; anchor-based `N+ years` labels are derived from the artifact as-of date and stale labels fail validation.
...
- `rules/18-opportunity-search-breadth-telemetry.md` defines canonical discovery stages, configurable breadth targets, named-source attempt records, approved substitutions, independent completion gates, rolling rotation, expansion-pass tracking, explicit stop conditions, and reconciled run telemetry.
...
- Fit-calibrated positioning preserves comprehensive internal gap analysis while ordering external evidence according to actual alignment, leading with source-backed value in highly aligned opportunities and surfacing material gaps proportionately in moderate- or low-alignment opportunities.
- Role-tailored resumes use the Resume Advancement Objective: establish qualification fast, then optimize truthful evidence selection and presentation for differentiation, source-backed uncertainty reduction, narrative coherence, credibility, reviewer interest, and next-stage advancement.
- Resume advancement optimization changes selection, wording, ordering, placement, and compression only. It does not create evidence, strengthen claim depth, hide a hard-screen failure, or authorize numeric conversion claims.
...
- `rules/22-criterion-evidence-recoverability.md` requires a post-draft audit of supported hard screens, load-bearing qualifications, and material central responsibilities so important truthful evidence is independently recoverable from the finished resume at the approved claim depth and proportionately visible in the initial reviewer decision.
- Criterion recoverability is vendor-neutral: exact keywords do not substitute for contextual proof, semantic equivalence may satisfy recognizability when unmistakable, Core Skills is an index rather than automatic proof, and unsupported target criteria remain unsupported.
- Recoverability revision may improve selection, wording, target-accurate terminology, placement, first-scan emphasis, decision salience, and non-redundant coverage, but it must stop before human readability, TruthGuard, differentiation, or claim-depth precision is degraded and must never invent an ATS score, candidate rank, reranker score, embedding similarity, resume-conversion probability, or advancement probability.
...
- Role-tailored resume optimization must preserve the Resume Advancement Objective together with vendor-neutral criterion evidence recoverability, without treating keyword presence as proof, inventing proprietary ATS behavior, reporting numeric conversion probability, or sacrificing human readability for speculative machine scoring
...
- Professional-effectiveness matching must remain source-grounded, behavior-based, and non-diagnostic; it must not create personality ratings, turn target adjectives into candidate evidence, or compensate for missing factual, technical, credential, or eligibility requirements
- Interview preparation may compress verified material for recall, but compression must not alter source authority, claim depth, do-not-claim boundaries, interviewer lens, or the distinction between a general approach and a concrete source-backed example
...
- Verified Opportunity Search must keep result limits separate from search breadth, evaluate numeric breadth, source coverage, title-family coverage, and telemetry reconciliation independently, and must not claim verified coverage without canonical opportunity and source-attempt records
