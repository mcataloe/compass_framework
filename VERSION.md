...
Current COMPASS Version: vNext 2026-09.1
...
- `rules/22-criterion-evidence-recoverability.md` requires a post-draft audit of supported hard screens, load-bearing qualifications, and material central responsibilities so important truthful evidence is independently recoverable from the finished resume at the approved claim depth.
...
- Recoverability revision may improve selection, wording, target-accurate terminology, placement, and non-redundant coverage, but it must stop before human readability, TruthGuard, differentiation, or claim-depth precision is degraded and must never invent an ATS score, candidate rank, reranker score, embedding similarity, or advancement probability.
...
- Role-tailored resume optimization must preserve vendor-neutral criterion evidence recoverability without treating keyword presence as proof, inventing proprietary ATS behavior, or sacrificing human readability for speculative machine scoring
